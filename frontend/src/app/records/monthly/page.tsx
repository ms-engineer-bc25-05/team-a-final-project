...
export default function RecordsMonthlyPage(): JSX.Element {
  ...
  const apiUrl = shouldFetch
    ? `${baseUrl}/api/records/monthly?userId=${user.uid}`
    : null;
          ...
          <div className="mb-7 text-center">
            {isLoading ? (
              ...
              <p className="text-base text-[#2c4d63] flex items-center justify-center gap-2">
                ...
                今月 <span className="font-semibold">{records.length}件</span> 達成
              </p>
            )}
          </div>
              ...
              {records.map((r, idx) => (
                ...
                >
                  <p className="flex items-center gap-2 text-[16px] text-[#2c4d63]">
                    <Star size={18} strokeWidth={2.4} className="text-[#6BB7D6] shrink-0 translate-y-px" /> 
                    {r.title}
                  </p>
